...
async function main() {
  ...
  const HEICEntries = (await fs.readdir(IMG_DIR)).filter(
    (file) =>
  ...
  for (const entry of HEICEntries) {
    ...
    const sharpConvert = sharp(output).jpeg({
    ...
    const promises = [
      ...
      sharpConvert
        ...
        .toFile(`${outputDir}/${baseName}-thumb-title.jpg`),
      sharpConvert
        .resize(300, 200, { fit: "cover", position: "entropy" })
        .toFile(`${outputDir}/${baseName}-thumb-faces.jpg`),
    ];
