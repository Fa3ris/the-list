...
export const createLoaderForDirectory = (directory: string): LoaderModule => {
  return defineLoader({
    watch: [
      `${process.cwd()}/docs/${directory}/**/*.md`,
      `!${process.cwd()}/docs/${directory}/**/index.md`,
    ],
    load(watchedFiles): YamlMetadata[] {
      const data = watchedFiles.map((filePath) => {
        ...
        const resolvedPath = path.relative(
          `${process.cwd()}/docs/${directory}`,
          `${process.cwd()}/${filePath}`,
        );
        const yaml: YamlMetadata = {
          ...parsed.data,
          path: resolvedPath.replace(/\.md$/, ""),
          // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
          imgURL:
            (parsed.data.imgURL && `/${directory}/${parsed.data.imgURL}`) ||
            undefined,
        };
        return yaml;
      });
